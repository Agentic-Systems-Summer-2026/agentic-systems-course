"""Shared OU AI Sandbox client for all Build Challenges. Stdlib only — no pip installs.
...
Features you will need across the course:
  ...
  - model=...: route a call to a cheaper model (e.g. "gemma4-small")
"""
import hashlib
import json
import os
...
def _key() -> str:
    ...
    if not key:
        raise RuntimeError(
            "LITELLM_API_KEY is not set. In a Codespace it comes from your "
...
def chat(messages, model=DEFAULT_MODEL, max_tokens=700, temperature=0,
    ...
    """One chat completion against the OU Sandbox. Returns the assistant text."""
    ...
    for attempt in range(retries + 1):
        for base in (BASE + "/v1", BASE):
            try:
                ...
                if 400 <= e.code < 500:
                    raise RuntimeError(f"Sandbox rejected the request "
    ...
    raise RuntimeError(f"Sandbox unreachable after {retries + 1} attempts: {last}")
